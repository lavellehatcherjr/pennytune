"""Watchlist and change-tracking tests.

Assert against flag names a feature profile actually emits. Testing the alert
path with a name no code path produces proves nothing about whether alerting
works on real data.
"""
...
def test_diff_new_material_flag_and_score_drop() -> None:
    ...
    current = Snapshot("VRMX", "2026-06-12T00:00:00", 61, ["DILUTION-SHELF-LARGE"])
    diff = diff_snapshots(prior, current)
    assert diff.score_delta == -12
    assert "DILUTION-SHELF-LARGE" in diff.new_flags
    assert diff.is_alert
    assert any("DILUTION-SHELF-LARGE" in a for a in diff.alerts)
    assert any("score -12" in a for a in diff.alerts)


def test_diff_baseline_has_no_alerts() -> None:
    diff = diff_snapshots(None, Snapshot("X", "t", 70, ["DILUTION-SHELF-LARGE"]))
...
def test_record_snapshots_and_alert_banner(tmp_path: Path) -> None:
    ...
    wl.record_snapshot("VRMX", 61, ["DILUTION-SHELF-LARGE"], now=datetime(2026, 6, 12))
    entry = next(e for e in wl.list_entries() if e.ticker == "VRMX")
    assert entry.last_score == 61
    assert entry.score_delta == -12
    assert entry.alerts
    assert any("VRMX" in a and "DILUTION-SHELF-LARGE" in a for a in wl.alerts())
    wl.close()
